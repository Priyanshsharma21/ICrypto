...
const TVShow = () => {
  ...
  return (
    ...
    <LoadingBar
      ...
      progress={progress}
      />
      <Banner entertainmentType="fetchTopDrama2022" />

      <Row title="Watch Popular Drama" fetchUrl={requests.fetchDrama} isLargeRow={true} setProgress={(progress)=>{setProgress(progress)}}/>
      <Row title="Horror Show's" fetchUrl={requests.fetchHorrorSeries} isLargeRow={true} setProgress={(progress)=>{setProgress(progress)}}/>
      <Row title="Top TV Show's" fetchUrl={requests.fetchAllTimeDrama} isLargeRow={true} setProgress={(progress)=>{setProgress(progress)}}/>
